...
export default function LightSwitch() {
  ...
  useEffect(() => {
    ...
    mqttClient.on("connect", () => {
      ...
      mqttClient.subscribe("smartlab/lampu", (err) => {
        if (!err) {
          console.log("Subscribed to smartlab/lampu");
        }
      });
    });

    mqttClient.on("message", (topic, message) => {
      if (topic === "smartlab/lampu") {
        const newState = message.toString() === "ON";
        setIsOn(newState);
      }
    });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-4">
      ...
      <div
        ...
        style={{
          filter: isOn
            ? `drop-shadow(0 0 ${
                10 + glowIntensity * 20
              }px rgba(255, 230, 100, ${0.5 + glowIntensity * 0.5}))`
            : "none",
          transform: isOn ? "scale(1.05)" : "scale(1)",
        }}
      >
        <svg version="1.1" viewBox="0 0 511.996 511.996" className="w-48 h-48">
      ...
      {/* Toggle Switch */}
      <div
        className={`relative w-24 h-12 rounded-full p-1 cursor-pointer transition-all duration-500 ${
          isOn ? "bg-blue-500" : "bg-gray-400"
        }`}
        onClick={toggleSwitch}
      >
        <div
          className={`absolute w-10 h-10 rounded-full bg-white shadow-md transform transition-all duration-500 ${
      ...
      <div
        className={`mb-4 px-5 py-2 mt-5 rounded-md text-sm ${
